...
// SetupWebhookWithManager registers the conversion webhook for UserNotificationRule.
func SetupWebhookWithManager(mgr ctrl.Manager) error {
	if err := ctrl.NewWebhookManagedBy(mgr, &v1alpha1.UserNotificationRule{}).
		Complete(); err != nil {
...
// SetupGated adds a controller that reconciles UserNotificationRule managed resources.
func SetupGated(mgr ctrl.Manager, o tjcontroller.Options) error {
	o.Options.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			mgr.GetLogger().Error(err, "unable to setup reconciler", "gvk", v1alpha1.UserNotificationRule_GroupVersionKind.String())
		}
	}, v1alpha1.UserNotificationRule_GroupVersionKind)
	return nil
}
...
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	...
	eventHandler := handler.NewEventHandler(handler.WithDefaultRateLimiter(rl), handler.WithLogger(o.Logger.WithValues("gvk", v1alpha1.UserNotificationRule_GroupVersionKind)))
	ac := tjcontroller.NewAPICallbacks(mgr, xpresource.ManagedKind(v1alpha1.UserNotificationRule_GroupVersionKind), tjcontroller.WithEventHandler(eventHandler), tjcontroller.WithStatusUpdates(false))
	opts := []managed.ReconcilerOption{
		managed.WithExternalConnecter(
			tjcontroller.NewTerraformPluginFrameworkAsyncConnector(mgr.GetClient(), o.OperationTrackerStore, o.SetupFn, o.Provider.Resources["datadog_on_call_user_notification_rule"],
				...
				tjcontroller.WithTerraformPluginFrameworkAsyncMetricRecorder(metrics.NewMetricRecorder(v1alpha1.UserNotificationRule_GroupVersionKind, mgr, o.PollInterval)),
